...
def pull_runtime_information(redis_client):
    ...
    for key in redis_client.scan_keys("future:*"):
        # Only the bare future:{id} key is a hash; any extra ":suffix" is a collection key (set).
        if key.count(":") > 1:
            continue
...
def send_runtime_information(
    ...
    # Acknowledge only after the database transaction commits -- see
    # docs/TELEMETRY_POLLING.md for why cleanup waits on this flag.
