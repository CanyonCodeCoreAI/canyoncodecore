...
def _container_owns_dashboard_port() -> bool:
    try:
    ...
    return any(
        binding.get("HostPort") == "8080"
        and binding.get("HostIp") in {"127.0.0.1", "0.0.0.0", "::"}
        for binding in bindings
    )


def _port_8080_is_free() -> bool:
...
def validate(config_path: str) -> DashboardStack:
    ...
    if database_url is None:
        raise PhaseFailure("validate", "database.url is required")
    ...
    if not _container_owns_dashboard_port() and not _port_8080_is_free():
        raise PhaseFailure("validate", "port 8080 on 127.0.0.1 is already in use")

    return DashboardStack(database_url.strip(), state_dir, project_root)
...
def prepare(stack: DashboardStack) -> tuple[dict[str, str], str]:
    try:
        ...
        managed_env = {
            "CANYONOS_DATABASE_URL": managed_database_url,
...
def _redact_stack_text(text: str, stack: DashboardStack, managed_env: dict[str, str]) -> str:
    ...
    return redact_logs(
        redact_logs(text, stack.database_url, secret), managed_env["CANYONOS_DATABASE_URL"], secret
    )
...
def redact_logs(logs: str, database_url: str, jwt_secret: str) -> str:
    redacted = logs.replace(database_url, "[redacted]").replace(jwt_secret, "[redacted]")
