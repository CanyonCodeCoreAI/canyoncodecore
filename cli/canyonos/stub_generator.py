...
def generate_docker(
    ...
    # Copy general agent files. The local controller runtime (local_controller.py,
    # local_controller_frontend.py, gpu_metrics.py, compiled gRPC stubs, ...) is
    # no longer vendored here -- it's baked into the pre-built base image below.
    files_to_copy += [
    ...
    # ---- Dockerfile ------------------------------------------------------
    # TODO: replace with the real pre-built agent-base image once it's
    # published to the registry. That image is expected to already contain
    # local_controller.py, local_controller_frontend.py, gpu_metrics.py, and
    # the compiled gRPC stubs at /app.
    agent_basename = os.path.basename(agent_file)
    dockerfile = f"""# syntax=docker/dockerfile:1
FROM ghcr.io/canyoncode/ventis-agent-base:latest
...
def generate_workflow_docker(
    ...
    # The local controller runtime (local_controller.py, gpu_metrics.py,
    # session_logging.py, compiled gRPC stubs, ...) is no longer vendored
    # here -- it's baked into the pre-built base image below.
    files_to_copy += [
...
"""
    ...
    # ---- Dockerfile ------------------------------------------------------
    # TODO: replace with the real pre-built workflow-base image once it's
    # published to the registry. That image is expected to already contain
    # local_controller.py, gpu_metrics.py, session_logging.py, and the
    # compiled gRPC stubs at /app.
    dockerfile = f"""# syntax=docker/dockerfile:1
FROM ghcr.io/canyoncode/ventis-workflow-base:latest
COPY --from=ghcr.io/astral-sh/uv:latest /uv /uvx /bin/
