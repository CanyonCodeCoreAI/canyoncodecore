...
- **`returns` is documentation.** The stub generator never reads it. Its value is
  ...
  needs `json.loads`.
- **The filename is not part of the contract.** It only names the generated stub,
  `stubs/<basename>.py` — but it must differ from any source module the adapter
  imports, since the stub also lands flat at `/app/<basename>.py`.
...
- The REST route is `fn.__name__` — rename the function and the endpoint renames
  with it. There is no fixed `/main`.
- The request body is splatted in as kwargs after `_context` is popped off.
