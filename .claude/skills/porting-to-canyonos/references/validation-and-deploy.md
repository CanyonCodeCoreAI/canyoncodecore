...
`validate.py` reads the `.car` source copy, and installs each image's declared
requirements to load the module that image runs. It never builds an image or
serves a request, so a clean report is the absence of the gaps it checks -- not
a working deployment. It cannot see anything reached only at runtime, or
whether the first request returns an answer: an AttributeError on the first
call survives an exit-0 run.

Pass `--no-smoke` only when the install cannot run at all. It is the one check
that catches a dependency set which resolves but does not import, which is
otherwise found by `canyonos deploy` after the port is reported finished.
